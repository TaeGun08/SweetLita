...
public class TIuBookManager : MonoBehaviour
{
    ...
    private void bookOnOff()
    {
        ...
        if (Input.GetKeyDown(KeyCode.B) && questManager.PlayerMoveStop() == false)
        {
            ...
            gameManager.SetPlayerMoveStop(onOff);
    ...
    private void openItem()
    {
        if (itemCheck == true)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                if (itemsIndex[i] == 100)
                {
                    itemImages[0].sprite = itemSprites;
                    Image image = itemImages[0].transform.GetChild(0).GetComponent<Image>();
                    Color color = image.color;
                    color.a = 1;
                    image.color = color;
                    TMP_Text text = itemImages[0].transform.GetChild(1).GetComponent<TMP_Text>();
                    text.text = "치즈조각";
                }
                else if (itemsIndex[i] == 101)
                {
                    itemImages[1].sprite = itemSprites;
                    GameObject obj = itemImages[1].transform.GetChild(0).GetComponent<GameObject>();
                    obj.SetActive(false);
                }
            }
            itemCheck = false;
